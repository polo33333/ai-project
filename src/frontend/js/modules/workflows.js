...
window.deleteWorkflow = async function (id) { if (!await showUiConfirm(`Xóa workflow "${workflowName(id)}"?`, { title: 'Xóa workflow', confirmText: 'Xóa workflow', tone: 'danger' })) return; const response = await fetch(`/api/workflows/${encodeURIComponent(id)}`, { method: 'DELETE' }); if (!response.ok) return notifyWorkflow('Không thể xóa workflow', 'error'); notifyWorkflow('Đã xóa workflow', 'success'); await refreshWorkflowManager(); };
