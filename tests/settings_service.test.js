...
test('settings reject unknown keys, executable edits, injection and invalid ranges atomically', t => {
  const { service, file } = fixture(t, 'PORT=3000\n');
  for (const values of [{ NODE_OPTIONS: '--inspect' }, { QDRANT_EXE: 'bad.exe' }, { PORT: '3000\nEVIL=yes' }, { PORT: '99999' }, { AI_DOCUMENT_MIN_SCORE: '2' }, { QDRANT_URL: 'http://user:pass@localhost' }]) {
